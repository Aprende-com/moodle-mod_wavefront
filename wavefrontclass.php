...
/**
 * Main 3D model class with all model manipulations as methods
 *
 * @package   mod_model
...
define('MODEL_POS_HID', 2);
define('MODEL_POS_TOP', 1);
define('MODEL_POS_BOT', 0);
...
class model_image {
    ...
    public function __construct($storedfile, $gallery, $cm) {
        ...
        $this->imageurl = $CFG->wwwroot.'/pluginfile.php/'.$this->context->id.'/mod_model/model_files/'.
                           $this->storedfile->get_itemid().$this->storedfile->get_filepath().$this->storedfile->get_filename();
        $this->thumburl = $CFG->wwwroot.'/pluginfile.php/'.$this->context->id.'/mod_model/model_thumbs/0/'.
                           $this->storedfile->get_filepath().$this->storedfile->get_filename().'.png';

        $imageinfo = $this->storedfile->get_modelinfo();
    ...
    public function add_tag($tag) {
        ...
        return $DB->insert_record('model_model_meta', $imagemeta);
    }
